              ...
              {blogPosts.map((post, index) => (
                <div key={index} className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg shadow-md flex flex-col">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <span className="text-gray-700 dark:text-gray-300 font-medium">{post.date} - {post.author}</span>
                    <a href={post.link} className="text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 font-medium">Read More &gt;</a>
                  </div>
                </div>
              ))}
            </div>
